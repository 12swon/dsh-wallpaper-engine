...
function setUploadMeta(id, title) {
  try {
...
export function apply(ctx) {
  ...
  function buildInventory() {
    ...
    const uploads = enumerateUploads(uploadsDir).map((w) => ({
      id: w.id,
      title: typeof uploadMeta[w.id] === 'string' && uploadMeta[w.id].trim()
        ? uploadMeta[w.id] : w.id,
      type: w.type,
      playable: true,
    ...
    handler: (req, res) => {
      ...
      req.on('end', () => {
        ...
        try {
          ...
          const fileAbs = join(ensureUploadDir(), id + '.' + ext);
          writeFileSync(fileAbs, Buffer.concat(chunks));
          if (title) setUploadMeta(id, title);
          const type = ext === 'mp4' ? 'video' : 'image';
          const payload = {
            id,
            title: title || id,
