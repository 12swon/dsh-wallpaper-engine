...
- **液态玻璃设置页**（v0.3.1）：设置页升级为**一级设置页**（参照 dsh-web-ui-all 皮肤中心的设计），整页是一张可自定义的液态玻璃卡片 —— **配色**（6 种预设 + 自定义取色）与**玻璃透明度**（0–60%）即时生效、持久保存，按钮、滑块、选中卡片与玻璃高光统一跟随所选主题色。
...
- **卡片样式**：设置页顶部可切换缩略图卡片的两种样式 —— **经典（CD 风）**（原版 `aspect-ratio 16:9` 卡片，作者喜欢的复古观感）或 **新版（防重叠）**（重写后的固定高度卡片，图片绝对定位填满，任何浏览器都不会重叠；默认）。选择保存在浏览器 `localStorage`。
...
设置页顶部「外观」区控制整张设置卡片与玻璃面板的观感，参照 dsh-web-ui-all 皮肤中心的设计：

| 控件 | 作用 | 范围 | 默认 |
|---|---|---|---|
| **配色** | 主题色：按钮、滑块、选中卡片、徽标与玻璃高光统一跟随 | 6 预设 + 自定义取色 | `#4f8cff` 经典蓝 |
| **玻璃透明度** | 玻璃面板（设置卡片、输入栏、气泡、侧边栏）的透明度 | 0–60 % | 12 % |

> 透明度越高玻璃越"透"（壁纸颜色更清晰地透过面板），越低越接近实色；不支持 `backdrop-filter` 的浏览器自动回退到高不透明实色，保证文字可读。两者都即时生效并保存在浏览器 `localStorage`，刷新不丢。
